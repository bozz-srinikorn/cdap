...
const TABLE1 = 'one_thousand_cols';
const TABLE2 = 'ninety_nine_cols';
...
describe('Creating pipeline with joiner in pipeline studio', () => {
  ...
  it('Should be able to build a complex pipeline with joiner widget', () => {
    ...
    cy.get(
      `${dataCy("BigQueryTable-stage-expansion-panel")} ${dataCy(`${TABLE_FIELDS[0]}-field-selector-alias-textbox`)} input`
    )
      .clear()
  ...
  it('Should show appropriate message when preview has been stopped before data is generated', () => {
    ...
    cy.get(dataCy('preview-top-run-btn')).should('be.visible');
